...
(function() {
	...
	Spider.prototype.query = function moveSpider(target) {
		...
		spider.request(query, function(err, res, body) {
			spider.parseHtml(err, res, body, query);
		});
	...
	Spider.prototype.parseHtml = function htmlParser(err, res, body, currentQuery) {
		var spider = this,
		...
		if (true || !err && 200 >= res.statusCode && res.statusCode < 400) {
			...
			var data = spider.scraper(parsedBody, body, currentQuery);

			counts.items += data.items.length;
			data.page = ++counts.pages;

			debug('Sending data.');
			spider.emit('data', data, body, currentQuery);
			...
			if (data.more && !spider.urlTemplate.isStatic) {
				...
				spider.kill({
					code: 200,
					message: 'Spider finished. Info: ' + util.inspect(counts)
				});
			}
		}
		...
		// On Error
			...
			var message = {
				code: (err ? err.code : null) || res.statusCode || 500,
